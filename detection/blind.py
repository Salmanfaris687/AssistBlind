...
def perform_closest_detection(model: YOLO, frame: np.ndarray, box_annotator: sv.BoxAnnotator) -> tuple:
    ...
    return annotated_frame, last_detected_object
...
def blindFunction(cap):
    ...
    while True:
        annotated_frame, detected_object = perform_closest_detection(model, frame, box_annotator)
        ret, frame = cap.read()
        ...
        if detected_text:
            ...
            if objectvoice != detected_object:
                objectvoice = detected_object
                text_to_speech("object Detected" + objectvoice)
