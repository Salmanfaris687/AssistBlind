...
@app.get('/video_for_text')
async def video_for_text():
    # Video streaming route. Put this in the src attribute of an img tag
    return StreamingResponse(stream.gen_frames('text'), media_type="multipart/x-mixed-replace; boundary=frame")
...
async def home_page(request: Request):
    return templates.TemplateResponse("object_detection.html", {"request": request, "dtype": "text"})
